%========================= CUSTOM SET UP ==============
A=1.3; lam=100; N=512; D=1; h=0.02; %domain/grid size, diff par, timestep
g=@(u,A) u-(1+1i*A)*u.*(abs(u).^2); %nonlinear function
%========================= GENERIC SET UP =========================
x=(lam/N)*(1:N)'; [X,Y]=ndgrid(x,x); %{[X,Y,Z]=ndgrid(x,x,x)}
u=u0; %u=randn(N,N)*0.1; 
v=fftn(u); %random IC. {randn(N,N,N)}
...
Fr=zeros(N,1); %High frequencies for de-aliasing
...
% it seems that h = 0.02 is accurate enough.