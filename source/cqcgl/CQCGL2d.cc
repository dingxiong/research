 ...
 * A_t = Mu A + (Dr + Di*i) (A_{xx} + A_{yy}) + (Br + Bi*i) |A|^2 A + (Gr + Gi*i) |A|^4 A
 *
 ...
 */
inline int CQCGL2d::calNe(const double N){
...
void CQCGL2d::changeOmega(double w){
    Omega = w;
    L = dcp(Mu, -Omega) - dcp(Dr, Di) * (QKx.square().replicate(1, M).transpose() + 
					 QKy.square().replicate(1, N)); 
    L.middleRows(Mplus, Malias) = ArrayXXcd::Zero(Malias, N); 
 ...
 * to save memory
 */
void CQCGL2d::calCoe(const double h){
...
void 
CQCGL2d::oneStep(double &du, const bool onlyOrbit){
...
ArrayXXcd CQCGL2d::ZR(const Ref<const ArrayXcd> &z){
 ...
 */
double
CQCGL2d::adaptTs(bool &doChange, bool &doAccept, const double s){
    double mu = 1;
    doChange = true;
    doAccept = true;
...
void CQCGL2d::saveState(H5File &file, int id, const ArrayXXcd &a, 
 ...
 */
ArrayXXcd 
CQCGL2d::constETD(const ArrayXXcd &a0, const ArrayXXcd &v0, 
 ...
 */
ArrayXXcd
CQCGL2d::adaptETD(const ArrayXXcd &a0, const ArrayXXcd &v0, 
 ...
 */
ArrayXXcd 
CQCGL2d::intg(const ArrayXXcd &a0, const double h, const int Nt, const int skip_rate,
		     const bool doSaveDisk, const string fileName){
...
ArrayXXcd
CQCGL2d::aintg(const ArrayXXcd &a0, const double h, const double tend, 
 ...
 */
ArrayXXcd
CQCGL2d::intgv(const ArrayXXcd &a0, const ArrayXXcd &v0, const double h,
...
ArrayXXcd 
CQCGL2d::aintgv(const ArrayXXcd &a0, const ArrayXXcd &v0, const double h,
 ...
 */
void CQCGL2d::dealias(const int k, const bool onlyOrbit){
 ...
 * */
void CQCGL2d::NL(const int k, const bool onlyOrbit){
    dcp B(Br, Bi);
    dcp G(Gr, Gi);
...
ArrayXXcd CQCGL2d::unpad(const ArrayXXcd &v){
    int m = v.rows();
    int n = v.cols();
    assert(m == M && n % N == 0);
    int s = n / N;
...
ArrayXXcd CQCGL2d::pad(const ArrayXXcd &v){
    int m = v.rows();
    int n = v.cols();
    assert( n % Ne == 0 && m == Me);
    int s = n / Ne;
...
ArrayXd CQCGL2d::c2r(const ArrayXXcd &v){
    assert(v.rows() == Me && v.cols() == Ne);
    return Map<ArrayXd>((double*)&v(0,0), 2 * v.rows() * v.cols());
}

ArrayXXcd CQCGL2d::r2c(const ArrayXd &v){
 ...
 */
ArrayXXcd CQCGL2d::Fourier2Config(const Ref<const ArrayXXcd> &aa){
    ArrayXXcd ap = pad(aa);
    int s = ap.cols() / N;
    ArrayXXcd AA(M, N*s);
 ...
 */
ArrayXXcd CQCGL2d::Config2Fourier(const Ref<const ArrayXXcd> &AA){
    int s = AA.cols() / N;
    ArrayXXcd aa(M, N*s);
 ...
 */
ArrayXXcd CQCGL2d::velocity(const ArrayXXcd &a0){
    assert(a0.rows() == Me && a0.cols() == Ne);
    F[0].v1 = pad(a0);
    NL(0, true);
 ...
 */
ArrayXXcd CQCGL2d::velocityReq(const ArrayXXcd &a0, const double wthx,
 ...
 */
ArrayXXcd CQCGL2d::stab(const ArrayXXcd &a0, const ArrayXXcd &v0){
    assert(a0.rows() == Me && a0.cols() == Ne && v0.rows() == Me && v0.cols() == Ne);
    F[0].v1 = pad(a0);
    JF[0].v1 = pad(v0); 
 ...
 */
ArrayXXcd CQCGL2d::stabReq(const ArrayXXcd &a0, const ArrayXXcd &v0,
...
ArrayXXcd CQCGL2d::rotate(const Ref<const ArrayXXcd> &a0, const int mode, const double th1,
...
ArrayXXcd CQCGL2d::tangent(const Ref<const ArrayXXcd> &a0, const int mode){
    switch (mode) {
...
CQCGL2d::readReq(const string fileName, const string groupName){
